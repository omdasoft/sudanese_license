<?php $pageTitle = "قائمة مراكز الامتحانات المعتمده"; ?>
<?php include("ini.php"); //include initalization file wich include connection to databse file , header, head, navbar ?>
			...
			<nav aria-label="breadcrumb">
				  <ol class="breadcrumb">
					...
					<li class="breadcrumb-item active" aria-current="page">مراكز الامتحانات المعتمدة</li>
				  </ol>
			</nav>
			<!--start of all news section-->
			<section class="approvedexam-list">
				<div class="table-responsive">
					<table class="table table-striped  wow fadeInDown" data-wow-offset="0" data-wow-delay="0.3s">
					  ...
					  <tbody>
						<?php 
						$examID = 1;
						$stmt = $con->prepare("SELECT * FROM basic_data WHERE exam_cnt = ?"); // select all approved center from database
						...
						$page_data = $con->prepare("SELECT cnt_name,city,address,phone,email FROM basic_data WHERE exam_cnt = ? order by cntID DESC limit ".$per_page." OFFSET ".$offset."");
						$page_data->execute(array($examID));
				...
				<form method="post" action="createExm_pdf.php">  
